...
def _make_state(**overrides) -> dict:
    defaults: dict = {
        ...
        "batch_execute_output": "I did the thing",
...
class TestEvaluateBatchQualityAgent:
    def test_returns_verdict_and_judge_verdict(self, mocker):
        mock_deep_agent(
            mocker,
            "deepworkflow.app.workflows.file_batch_workflow.nodes.batch_evaluate_quality_agent.create_agent",
            _judge_response("OK"),
        )
        result = batch_evaluate_quality_agent(_make_state())
        assert result["batch_evaluate_level"] == JudgeLevel.OK
        judge = result["batch_evaluate_verdict"]
    ...
    def test_warning_verdict_propagated(self, mocker):
        ...
        result = batch_evaluate_quality_agent(_make_state())
        assert result["batch_evaluate_level"] == JudgeLevel.WARNING
        judge = result["batch_evaluate_verdict"]
    ...
    def test_invalid_json_returns_error_verdict(self, mocker):
        ...
        result = batch_evaluate_quality_agent(_make_state())
        assert result["batch_evaluate_level"] == JudgeLevel.ERROR
        judge = result["batch_evaluate_verdict"]
        assert isinstance(judge, JudgeVerdict)
    ...
    def test_multiple_findings_worst_verdict_wins(self, mocker):
        ...
        result = batch_evaluate_quality_agent(_make_state())
        assert result["batch_evaluate_level"] == JudgeLevel.ERROR
        assert len(result["batch_evaluate_verdict"].findings) == 2
