...
from deepworkflow.app.workflows.file_batch_workflow.nodes.map_resolve_step import _is_glob_pattern, map_resolve_step
...
class TestResolveGlobs:
    def test_explicit_paths_kept_as_is(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, ["a.py", "b.py"])})
            assert result == {"map_files": ["a.py", "b.py"]}

    def test_deduplication(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, ["a.py", "b.py", "a.py"])})
            assert result == {"map_files": ["a.py", "b.py"]}
    ...
    def test_glob_expansion(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = map_resolve_step({"config": _make_config(td, ["*.py"])})
            ...
            assert sorted(result["map_files"]) == ["bar.py", "foo.py"]

    def test_empty_result_returns_error(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, ["nonexistent_*.xyz"])})
            assert "error" in result

    def test_line_range_preserved(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, ["file.py:10-20"])})
            assert result == {"map_files": ["file.py:10-20"]}

    def test_task_files_none_returns_empty_list(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, None)})
...
class TestResolveGlobsExcludes:
    def test_exclude_explicit_path_removes_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = map_resolve_step({"config": _make_config(td, ["a.py", excl], task_files_exclude=[excl])})
            assert result == {"map_files": ["a.py"]}
    ...
    def test_exclude_glob_pattern_removes_matching_files(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            Path(td, "bar.py").touch()
            Path(td, "baz.txt").touch()
            result = map_resolve_step({"config": _make_config(td, ["*.py", "*.txt"], task_files_exclude=["*.txt"])})
            py_files = sorted(result["map_files"])
    ...
    def test_exclude_none_has_no_effect(self):
        with tempfile.TemporaryDirectory() as td:
            result = map_resolve_step({"config": _make_config(td, ["a.py", "b.py"], task_files_exclude=None)})
            assert result == {"map_files": ["a.py", "b.py"]}

    def test_exclude_with_line_range_entry_still_filtered(self):
        with tempfile.TemporaryDirectory() as td:
            excl = str(Path(td, "a.py"))
            result = map_resolve_step({"config": _make_config(td, [excl + ":10-20"], task_files_exclude=[excl])})
