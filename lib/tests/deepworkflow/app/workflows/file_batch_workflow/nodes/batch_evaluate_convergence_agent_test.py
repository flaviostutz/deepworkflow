...
def _make_state(config: DeepWorkflowConfig | None = None, **overrides) -> dict:
    ...
    defaults: dict = {
        ...
        "batch_cumulative_output": "prior pass did some work",
    }
...
class TestFirstPassBypass:
    def test_no_llm_call_on_first_pass(self):
        """When batch_cumulative_output is absent, no LLM is invoked."""
        called: list[str] = []
        ...
        state = _make_state(config=config, batch_cumulative_output="")
        result = batch_evaluate_convergence_agent(state)

        assert called == [], "LLM must not be invoked on first pass"
        assert result["batch_evaluate_convergence_output"] == ""

    def test_first_pass_returns_warning_verdict(self):
        config, _ = _make_config()
        state = _make_state(config=config, batch_cumulative_output="")
        result = batch_evaluate_convergence_agent(state)
        verdict = result["batch_evaluate_convergence_verdict"]
    ...
    def test_first_pass_missing_key_also_bypasses(self):
        """State with no batch_cumulative_output key at all is treated as first pass."""
        config, _ = _make_config()
        state = {
            "config": config,
            "batch_current_index": 0,
        ...
        result = batch_evaluate_convergence_agent(state)
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.WARNING
...
class TestEvaluateBatchConvergenceAgent:
    def test_returns_warning_verdict_when_not_converged(self):
        ...
        result = batch_evaluate_convergence_agent(_make_state(config=config))
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.WARNING
    ...
    def test_returns_ok_verdict_when_converged(self):
        ...
        result = batch_evaluate_convergence_agent(_make_state(config=config))
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.OK
    ...
    def test_returns_error_verdict_on_invalid_json(self):
        ...
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.ERROR
    ...
    def test_uses_correct_agent_name(self):
        ...
        batch_evaluate_convergence_agent(_make_state(config=config))
        assert captured == ["batch_evaluate_convergence_agent"]
    ...
    def test_includes_execute_messages_in_conversation(self):
        ...
        result = batch_evaluate_convergence_agent(_make_state(config=config, execute_messages=[prior_msg]))
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.WARNING

    def test_empty_execute_messages_still_evaluates(self):
        config, _ = _make_config(_judge_response("OK"))
        result = batch_evaluate_convergence_agent(_make_state(config=config, execute_messages=[]))
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.OK
    ...
    def test_raw_output_preserved(self):
        ...
        result = batch_evaluate_convergence_agent(_make_state(config=config))
        assert result["batch_evaluate_convergence_output"] == raw
...
class TestEvaluateBatchConvergenceVerdict:
    ...
    def test_multiple_findings_worst_verdict_wins(self):
        ...
        result = batch_evaluate_convergence_agent(_make_state(config=config))
        assert result["batch_evaluate_convergence_verdict"].verdict == JudgeLevel.WARNING
        assert len(result["batch_evaluate_convergence_verdict"].findings) == 2
    ...
    def test_finding_fields_populated_correctly(self):
        ...
        config, _ = _make_config(raw)
        result = batch_evaluate_convergence_agent(_make_state(config=config))
        f: JudgeFinding = result["batch_evaluate_convergence_verdict"].findings[0]
