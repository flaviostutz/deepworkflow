...
class TestIncrementRetryStep:
    def test_increments_retry_count_from_zero(self):
        result = increment_retry_step(_make_state())
        assert result["retry_count"] == 1

    def test_increments_retry_count_from_nonzero(self):
        result = increment_retry_step(_make_state(retry_count=2))
        assert result["retry_count"] == 3

    def test_resets_batch_repeat_count(self):
        result = increment_retry_step(_make_state(batch_repeat_count=5))
        assert result["batch_repeat_count"] == 0

    def test_increments_quality_retries_when_within_limit(self):
        effort = EffortConfig(evaluate_batch_quality_max_retries=3)
        stats = WorkflowStats()
        token = _stats_var.set(stats)
        try:
            increment_retry_step(_make_state(retry_count=0, effort_config=effort))
    ...
    def test_does_not_increment_quality_retries_when_exceeds_limit(self):
        effort = EffortConfig(evaluate_batch_quality_max_retries=1)
        stats = WorkflowStats()
        token = _stats_var.set(stats)
        try:
            increment_retry_step(_make_state(retry_count=1, effort_config=effort))
    ...
    def test_no_effort_config_treats_max_retries_as_zero(self):
        ...
        token = _stats_var.set(stats)
        try:
            increment_retry_step(_make_state(retry_count=0, effort_config=None))
