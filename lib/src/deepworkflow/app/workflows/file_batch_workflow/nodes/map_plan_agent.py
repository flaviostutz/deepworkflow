...
DEFAULT_JUDGE_BATCH_INSTRUCTIONS = (
    ...
    "- Output COULD include additional context or clarifications that improve readability, but only when they do not conflict with the instructions."  # noqa: E501
)
...
_ROLE = """\
You are the `map_plan_agent`. You are an expert at partitioning large file sets into balanced,
...
3. {discover_or_group}
4. Produce a `map_plan_overview` — a high-level strategy and context that all batches should follow.
   Use MUST/SHOULD/COULD language to express quality requirements that apply to all batches.
5. Produce `reduce_instructions` — how to merge/consolidate results from all batches at the end.
...
- If the task involves creating or modifying output files, include the full list of expected output
  files in `map_plan_overview` (shared across all batches) or in `batch_instructions` (batch-specific).
- `map_plan_overview` MUST provide enough shared context, target outputs, or guidelines for all batches
...
def map_plan_agent(state: file_batch_workflow_state) -> dict:
    """Plan batch definitions using a read-only agent."""
    config = state["config"]
    effort_config = state["effort_config"]
    task_files = state["map_files"]

    if effort_config.batch_evaluate_quality_instructions is not None:
        words = set(effort_config.batch_evaluate_quality_instructions.lower().split())
        if not words & _MANDATORY_ADVISORY_KEYWORDS:
            return {
                "error": (
                    "batch_evaluate_quality_instructions must contain at least one MANDATORY/ADVISORY keyword "
    ...
    evaluate_quality_feedback_section = ""
    map_verdict = state.get("map_evaluate_verdict")
    if map_verdict is not None and map_verdict.findings:
        feedback_lines = [
            f"- [{f.level.name}] {f.title}"
            + (f": {f.reason}" if f.reason else "")
            + (f" | Fix: {f.fix}" if f.fix else "")
            for f in map_verdict.findings
        ]
        evaluate_quality_feedback_section = (
            "Previous batch plan was rejected by map_evaluate_agent. Address this feedback:\n"
            + "\n".join(feedback_lines)
        )
    ...
    agent = create_agent(
        model=config.model("map_plan_agent"),
    ...
    parsed["batch_evaluate_quality_instructions"] = _derive_evaluate_quality_instructions(config, effort_config)
    return parsed


def _derive_evaluate_quality_instructions(config, effort_config) -> str:  # type: ignore[misc]  # noqa: ANN001
    """Return batch_evaluate_quality_instructions from effort_config or derive them via LLM."""
    if effort_config.batch_evaluate_quality_instructions is not None:
        return effort_config.batch_evaluate_quality_instructions
...
def _parse_map_output(content: str) -> dict:
    ...
    try:
        ...
        return {"error": f"map_plan_agent produced invalid JSON output: {content[:200]}"}
    ...
    map_plan_overview = data.get("map_plan_overview", "")
    reduce_instructions = data.get("reduce_instructions", "")
    raw_batches = data.get("batches", [])

    if not raw_batches:
        return {"error": "map_plan_agent produced zero batches."}
