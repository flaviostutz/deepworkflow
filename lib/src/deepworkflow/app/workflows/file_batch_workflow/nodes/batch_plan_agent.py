...
_ROLE = """\
You are the `batch_plan_agent`. You are an expert at analysing a file set and designing a precise,
...
def batch_plan_agent(state: file_batch_workflow_state) -> dict:
    """Plan the execution for the current batch.

    Creates a fresh agent each time (including retries):
    'spawns a brand new agent thread with only task context + batch_evaluate_verdict feedback'.
    """
    config = state["config"]
    ...
    agent = create_agent(
        model=config.model("batch_plan_agent"),
