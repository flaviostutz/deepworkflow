...
_ROLE = """\
You are the `batch_evaluate_quality_agent`. You run once after all execution passes for a batch
complete and decide whether the result meets the required quality bar — not whether to repeat
(that is the batch_evaluate_convergence_agent's responsibility)."""
...
_GUARDRAILS = """\
- Do NOT decide whether to repeat passes — that is the batch_evaluate_convergence_agent's responsibility.
...
def batch_evaluate_quality_agent(state: file_batch_workflow_state) -> dict:
    ...
    evaluate_quality_instructions = state["batch_evaluate_quality_instructions"]
    ...
    agent = create_agent(
        model=config.model("batch_evaluate_quality_agent"),
    ...
    return {
        "batch_evaluate_level": judge_verdict.verdict,
        "batch_evaluate_verdict": judge_verdict,
    }
