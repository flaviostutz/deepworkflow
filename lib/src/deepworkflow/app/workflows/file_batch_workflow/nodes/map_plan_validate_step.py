...
def map_plan_validate_step(state: file_batch_workflow_state) -> dict:  # noqa: C901, PLR0912
    """Deterministic validation of the batch plan produced by map_plan_agent or map_plan_step.
    ...
    5. When map_files was provided: every file is assigned to exactly one batch (coverage +
    ...
    task_files: list[str] = state.get("map_files") or []
    batches: list[BatchDefinition] = state.get("map_batches") or []
    ...
    if errors:
        return {"error": "map_plan_validate_step: " + " | ".join(errors)}
