...
_ROLE = """\
You are the `batch_execute_agent`. You are an expert at carrying out a well-defined plan precisely,
...
def batch_execute_agent(state: file_batch_workflow_state) -> dict:
    ...
    task_overview = state.get("map_plan_overview", "")
    ...
    cumulative_files_read = state.get("batch_cumulative_files_read", [])
    cumulative_files_written = state.get("batch_cumulative_files_written", [])
    ...
    agent = create_agent(
        model=config.model("batch_execute_agent"),
    ...
    # Store messages for batch_reflect_agent to continue the conversation
    messages = result.get("messages", [])

    return {"batch_execute_output": execute_output, "batch_execute_messages": messages}
