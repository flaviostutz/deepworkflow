...
_ROLE = """\
You are the `batch_reflect_agent`. You are a precise observer who analyses the execution
...
def batch_reflect_agent(state: file_batch_workflow_state) -> dict:
    """Reflect on execution to identify files read and written.

    Continues the same agent thread from batch_execute_agent by sending
    ...
    model = config.model("batch_reflect_agent")
    ...
    return {"batch_files_read": files_read, "batch_files_written": files_written}
